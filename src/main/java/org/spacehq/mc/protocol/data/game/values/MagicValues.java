...
import java.util.*;
import org.spacehq.mc.protocol.data.game.values.setting.*;
import org.spacehq.mc.protocol.data.game.values.entity.*;
import org.spacehq.mc.protocol.data.game.values.window.*;
import org.spacehq.mc.protocol.data.game.values.window.property.*;
...
import org.spacehq.mc.protocol.data.game.values.statistic.*;
...
import org.spacehq.mc.protocol.data.game.values.world.*;

public class MagicValues
{
    private static final Map<Enum<?>, Object> values;

    private static void register(final Enum<?> key, final Object value) {
        MagicValues.values.put(key, value);
    }

    public static <T extends Enum<?>> T key(final Class<T> keyType, final Object value) {
        for (final Enum<?> key : MagicValues.values.keySet()) {
            final Object val = MagicValues.values.get(key);
            if (keyType.isAssignableFrom(key.getClass())) {
                ...
                final Number num2 = (Number)value;
    ...
    public static <T> T value(final Class<T> valueType, final Enum<?> key) {
        final Object val = MagicValues.values.get(key);
        if (val != null) {
    ...
    static {
        values = new HashMap<Enum<?>, Object>();
